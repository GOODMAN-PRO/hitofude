...
foreach ($n in $nums) {
    ...
    if ($p.splash) {
        ...
        if ($p.panels -lt 6 -or $p.panels -gt 8) { $violations += "$id`: $($p.panels) panels — must be 6-8 (default 7)" }
        if ($p.bubbles -lt 8 -or $p.bubbles -gt 14) { $violations += "$id`: $($p.bubbles) bubbles — must be 8-14 (default 10-12)" }
    }
